...
# Пример:
try:
    # если попробовать открыть файл в режиме чтения 'r',
    f = open('testfile', 'w', encoding='utf-8')
    # то на экран будет выведено сообщение:
...
# Если мы хотим выполнить какой-то код после обработки исключения,
# то для этой цели можно использовать finally.

# finally

# Блок кода finally: выполняется всегда, вне зависимости от того,
# возникало или нет какое-либо исключение в блоке try.
# Синтаксис следующий:
# try:
#    Блок кода
#    ...
...
# При попытке ввести не число мы получим ошибку во время вывода на экран переменной val

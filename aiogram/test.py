...
url_btn1 = InlineKeyboardButton(
    text="Link 1",
    url="https://www.youtube.com/watch?v=gpCIfQUbYlY&list=PLNi5HdK6QEmX1OpHj0wvf8Z28NYoV5sBJ&index=9&ab_channel=PythonHubStudio",
)
url_btn2 = InlineKeyboardButton(
    text="link 2",
    url="https://www.youtube.com/watch?v=_r0vlyp33pw&ab_channel=dreamscape",
)
url_kb.row(url_btn1, url_btn2)

clb_kb = InlineKeyboardMarkup(row_width=2)
for product in dbase.get_products():
    clb_btn = InlineKeyboardButton(
        text=product[1], callback_data=f"product-{product[1]}"
    )
